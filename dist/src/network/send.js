...
function sendMessage(params) {
    ...
    if (isTribeOwner && !isForwarded) {
        theSender = {}(sender.dataValues || sender),
            role;
        constants.chat_roles.owner,
        ;
    }
    ;
}
...
await;
asyncForEach(contactIds, async(contactId), {
...
export function signAndSend(opts, owner, mqttTopic, replayingHistory) {
    ...
    return new Promise(async, function (resolve, reject) {
        ...
        try {
            if (mqttTopic) {
                await;
                tribes.publish(mqttTopic, data, ownerPubkey, function (err) {
                    if (!replayingHistory) {
                        if (mqttTopic)
...
export function newmsg(type, chat, sender, message, isForwarded, includeStatus) {
    const includeGroupKey = type === constants.message_types.group_create ||
    ...
    sender;
{
    ...
    (sender.routeHint && { route_hint: sender.routeHint }),
        alias;
    includeAlias ? aliasToInclude : "",
        role;
    sender.role || constants.chat_roles.reader,
    ;
