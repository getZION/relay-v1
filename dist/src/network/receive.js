...
import { encryptTribeBroadcast } from "../utils/msg";
...
function onReceive(payload, dest) {
    ...
    if (isTribeOwner && typesToForward.includes(payload.type)) {
        ...
        payload = await;
        uniqueifyAlias(payload, senderContact, chat, owner);
        if (doAction)
...
function doTheAction(data, owner) {
    ...
    if (payload.isTribeOwner) {
        ...
        const me = owner;
        payload = await;
        encryptTribeBroadcast(pld, me, true); // true=isTribeOwner
        if (ogContent)
...
function forwardMessageToTribe(ogpayload, sender, realSatsContactId, amtToForwardToRealSatsContactId, owner) {
    ...
    if (sender && typesToModify.includes(ogpayload.type)) {
        payload = await;
        modifyPayloadAndSaveMediaKey(ogpayload, chat, sender, owner);
    }
    else {
        payload = ogpayload;
    }
    ...
    sendMessage({
        type,
        message,
        sender: {} }, ...owner.dataValues, alias, (payload.sender && payload.sender.alias) || "", photoUrl, (payload.sender && payload.sender.photo_url) || "", role, constants.chat_roles.reader);
}
...
function initGrpcSubscriptions() {
    try {
        await;
        lightning.getInfo(true); // try proxy
        await;
        lndService.subscribeInvoices(parseKeysendInvoice);
    }
    catch (e) {
        throw e;
    }
    ...
    try {
        ...
        if (payload && payload.sender && payload.sender.pub_key) {
            ...
            if (sig.length === 96 && payload.sender.pub_key) {
                v = await;
                signer.verifyAscii(msg, sig, payload.sender.pub_key);
            }
            if (sig.length === 104) {
                v = await;
                lightning.verifyAscii(msg, sig);
            }
            if (v && v.valid) {
                return payload;
            }
...
function parseKeysendInvoice(i) {
    ...
    if (isProxy()) {
        try {
            const invoice = await, decodePayReq = (i.payment_request);
            if (!invoice)
        ...
        owner = await;
        models.Contact.findOne({ where: { isOwner: true } });
