...
export const findOrCreateChat = async(params);
{
    ...
    if (chat_id) {
        chat = await;
        models.Chat.findOne({ where: { id: chat_id, tenant: owner_id } });
    }
    else {
        ...
        // find by uuid
        chat = await;
        models.Chat.findOne({ where: { uuid, tenant: owner_id } });
        if (!chat) {
...
export const sendContactKeys = async({ type, contactIds, sender, success, failure, dontActuallySendContactKey, contactPubKey, routeHint }, { type: number, contactIds: number[], sender: any, success: Function, failure: Function, dontActuallySendContactKey: boolean, contactPubKey: string, routeHint: string });
{
    ...
    let cids = contactIds || [];
    await;
    asyncForEach(cids, async, contactId => {
        let destination_key;
...
function parseReceiveParams(payload) {
    ...
    if (isConversation) {
        sender = await;
        findOrCreateContactByPubkeyAndRouteHint(sender_pub_key, sender_route_hint, owner.dataValues);
        chat = await;
        findOrCreateChatByUUID(chat_uuid, [parseInt(owner.id), parseInt(sender.id)], owner.id);
        if (sender.fromGroup) {
            await;
            sender.update({ fromGroup: false });
        }
    }
    else {
        sender = await;
        models.Contact.findOne({ where: { publicKey: sender_pub_key, tenant: owner.id } });
        // inject a "sender" with an alias
        if (!sender && chat_type == constants.chat_types.tribe) {
            sender = { id: 0, alias: sender_alias };
        }
        chat = await;
        models.Chat.findOne({ where: { uuid: chat_uuid, tenant: owner.id } });
    }
