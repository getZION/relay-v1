...
export const checkRoute = async(req, res);
{
    ...
    if (!(pubkey && pubkey.length === 66))
        return failure(res, "wrong pubkey");
...
export const checkRouteByContactOrChat = async(req, res);
{
    ...
    if (!chatID && !contactID)
        return failure(res, "no chat_id or contact_id");
    let pubkey = "";
    let routeHint = "";
    if (contactID) {
        ...
        if (!contact)
            return failure(res, "cant find contact");
...
function getLogsSince(req, res) {
    ...
    let err;
    await;
    asyncForEach(logFiles, async(filepath), {
        if() { } }, !txt);
    {
        try {
...
function clearForTesting(req, res) {
    ...
    try {
        await;
        models.Chat.destroy({ truncate: true, where: { tenant } });
        await;
        models.Subscription.destroy({ truncate: true, where: { tenant } });
        await;
        models.Accounting.destroy({ truncate: true, where: { tenant } });
        await;
        models.Bot.destroy({ truncate: true, where: { tenant } });
        await;
        models.BotMember.destroy({ truncate: true, where: { tenant } });
        await;
        models.ChatBot.destroy({ truncate: true, where: { tenant } });
        await;
        models.Invite.destroy({ truncate: true, where: { tenant } });
        await;
        models.MediaKey.destroy({ truncate: true, where: { tenant } });
        await;
        models.Message.destroy({ truncate: true, where: { tenant } });
        await;
        models.Timer.destroy({ truncate: true, where: { tenant } });
        await;
