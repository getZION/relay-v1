...
function kickChatMember(req, res) {
    ...
    await;
    chat.update({ contactIds: JSON.stringify(newContactIds) });
    // remove from ChatMembers
    await;
    models.ChatMember.destroy({
        where: {
            chatId,
            contactId,
            tenant,
...
function createGroupChat(req, res) {
    ...
    await;
    asyncForEach(contact_ids, async(cid), {
    ...
    if (is_tribe) {
        chatParams = await;
        createTribeChatParams(owner, contact_ids, name, img, price_per_message, price_to_join, escrow_amount, escrow_millis, unlisted, req.body.private, app_url, feed_url, tenant);
        if (chatParams.uuid) {
...
sender: owner,
    type;
constants.message_types.group_create,
    message;
{ }
failure: function (e) {
    failure(res, e);
...
function addGroupMembers(req, res) {
    ...
    await;
    asyncForEach(allContactIds, async(cid), {
        const: contact = await, models: .Contact.findOne({
            where: { id: cid, tenant },
        }),
        if(contact) {
...
sender: owner,
    type;
constants.message_types.group_invite,
    message;
{ }
;
export const deleteChat = async(req, res);
{
    ...
    await;
    models.Message.destroy({ where: { chatId: id, tenant } });
    await;
    models.ChatMember.destroy({ where: { chatId: id, tenant } });
...
function receiveGroupJoin(payload) {
    ...
    const { owner, chat, sender_pub_key, sender_alias, chat_members, chat_type, isTribeOwner, date_string, network_type, sender_photo_url, sender_route_hint, chat_name, } = await, helpers, parseReceiveParams = (payload);
    const tenant = owner.id;
    if (!chat)
        return;
    ...
    if (!isTribe || isTribeOwner) {
        ...
        if (sender) {
            ...
            if (member && member.key) {
                if (sender.contactKey !== member.key) {
                    await;
                    sender.update({ contactKey: member.key });
                }
            }
        }
        else {
        ...
        await;
        chat.update({ contactIds: JSON.stringify(contactIds) });
        if (isTribeOwner) {
...
function receiveGroupLeave(payload) {
    ...
    const { chat, owner, sender_pub_key, chat_type, sender_alias, isTribeOwner, date_string, network_type, sender_photo_url, chat_name, } = await, helpers, parseReceiveParams = (payload);
    const tenant = owner.id;
    if (!chat)
        return;
    const isTribe = chat_type === constants.chat_types.tribe;
...
function receiveGroupCreateOrInvite(payload) {
    ...
    for (let [pubkey, member] of Object.entries(chat_members)) {
        ...
        if (chat_type === constants.chat_types.group && member && member.key) {
            ...
            if (member.role === constants.chat_roles.owner ||
