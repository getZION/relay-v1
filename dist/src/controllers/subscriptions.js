...
function getRawSubs(opts = {}) {
    const options = { order: [["id", "asc"]], opts };
    try {
...
function createSubscription(req, res) {
    ...
    const date = new Date();
    date.setMilliseconds(0);
    const s = jsonToSubscription({}, ...req.body, count, 0, total_paid, 0, createdAt, date, ended, false, paused, false, tenant);
}
;
if (!s.cron) {
...
function editSubscription(req, res) {
    ...
    date.setMilliseconds(0);
    const id = parseInt(req.params.id);
    const s = jsonToSubscription({}, ...req.body, count, 0, createdAt, date, ended, false, paused, false, tenant);
}
;
try {
