...
import { parseLDAT, urlBase64FromBytes } from "../utils/ldat";
...
export const sendAttachmentMessage = async(req, res);
{
    ...
    // }
    const { chat_id, contact_id, muid, text, remote_text, remote_text_map, media_key_map, media_type, amount, file_name, ttl, price, reply_uuid, } = req.body;
...
const mediaTerms = {
    muid,
    ttl: TTL,
    meta: {}(amt && { amt }) }, skipSigning;
true;
false,
; // only sign if its free
;
const msg = {
    mediaTerms,
    id: message.id,
...
network.sendMessage({
    chat: {} }, ...chat.dataValues, contactIds, [sender.id], , sender, owner, type, constants.message_types.purchase_accept, message, msgToSend, success, async(data), {
...
function signer(req, res) {
    ...
    try {
        const sig = await, signBuffer = (Buffer.from(req.params.challenge, "base64"),
            req.owner.publicKey);
