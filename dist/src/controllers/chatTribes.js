...
import { decryptMessage } from "../utils/msg";
...
function joinTribe(req, res) {
    ...
    network.sendMessage({
        ...
        chat: {} }, ...chatParams, contactIds, contactIdsToSend, members, {
        [owner.publicKey]: {
...
function receiveMemberRequest(payload) {
    ...
    const { owner, chat, sender_pub_key, sender_alias, chat_members, chat_type, isTribeOwner, network_type, sender_photo_url, sender_route_hint, } = await, helpers, parseReceiveParams = (payload);
...
function approveOrRejectMember(req, res) {
    ...
    if (status === "approved") {
        ...
        await;
        member.update({ status: memberStatus });
    }
...
function receiveMemberApprove(payload) {
    ...
    if (!chat)
        return console.log("no chat");
    await;
    chat.update({ status: constants.chat_statuses.approved });
    const tenant = owner.id;
    let date = new Date();
    date.setMilliseconds(0);
    const msg = {
...
function receiveMemberReject(payload) {
    ...
    if (!chat)
        return console.log("no chat");
    await;
    chat.update({ status: constants.chat_statuses.rejected });
    const tenant = owner.id;
    let date = new Date();
    date.setMilliseconds(0);
    const msg = {
...
function replayChatHistory(chat, contact, ownerRecord) {
    ...
    let msg = network.newmsg(m.type, chat, sender, {
        ...
        status: m.status,
        amount: m.amount, }, ...(mediaKeyMap && { mediaKey: mediaKeyMap }), ...(newMediaTerms && { mediaToken: newMediaTerms }), ...(m.mediaType && { mediaType: m.mediaType }), ...(dateString && { date: dateString }));
}
isForwarded,
    includeStatus;
;
msg = await;
decryptMessage(msg, chat);
