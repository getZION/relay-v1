...
export const getContactsForChat = async(req, res);
{
    ...
    if (!contacts)
        return failure(res, "no contacts found");
    ...
    chats.map(async(chat), {
        if(chat, type =  === constants.chat_types.conversation) {
            ...
            if (contactIds.includes(id)) {
                ...
                await;
                models.Message.destroy({ where: { chatId: chat.id, tenant } });
            }
        }
    });
    await;
    models.Invite.destroy({ where: { contactId: id, tenant } });
    await;
    models.Subscription.destroy({ where: { contactId: id, tenant } });
    success(res, {});
}
;
