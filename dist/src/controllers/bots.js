...
function keysendBotInstall(b, chat_uuid, owner) {
    return await;
    botKeysend(constants.message_types.bot_install, b.botUuid, b.botMakerPubkey, b.pricePerUse, chat_uuid, owner, "", undefined, b.botMakerRouteHint);
}
async;
function keysendBotCmd(msg, b, owner) {
    ...
    return await;
    botKeysend(constants.message_types.bot_cmd, b.botUuid, b.botMakerPubkey, amt, msg.chat.uuid, owner, msg.message.content, msg.sender && msg.sender.role, b.botMakerRouteHint);
}
async;
...
function receiveBotInstall(payload) {
    ...
    if (verifiedOwnerPubkey === owner.publicKey) {
        ...
        await;
        models.BotMember.create(botMember);
    }
...
export function buildBotPayload(msg) {
