...
function migrate() {
    ...
    try {
        await;
        sequelize.query(`CREATE UNIQUE INDEX chat_bot_index ON sphinx_chat_bots(chat_id, bot_uuid);`);
    }
    catch (e) { }
    ...
    try {
        await;
        sequelize.query(`CREATE UNIQUE INDEX chat_member_index ON sphinx_chat_members(chat_id, contact_id);`);
    }
    catch (e) { }
    ...
    try {
        await;
        sequelize.query(`CREATE INDEX idx_messages_sender ON sphinx_messages (sender);`);
    }
    catch (e) { }
...
function addTenant(tableName) {
    ...
    try {
        await;
        sequelize.query(`update ${tableName} set tenant=1 where tenant IS NULL`);
    }
    catch (e) {
        console.log(e);
    }
...
function addTableColumn(table, column, type = 'TEXT') {
    try {
        await;
        sequelize.query(`alter table ${table} add ${column} ${type}`);
    }
    catch (e) {
    }
}
