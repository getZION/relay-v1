...
function setVersion() {
    try {
        await;
        sequelize.query(`PRAGMA user_version = ${USER_VERSION}`);
    }
    catch (e) {
...
const runMigrations = async();
{
    ...
    new Promise((resolve, reject) => {
        const migration = exec('node_modules/.bin/sequelize db:migrate', { env: process.env }, (err, stdout, stderr) => {
            if (err) {
                reject(err);
            }
            else {
