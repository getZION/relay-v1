...
function initializeClient(pubkey, host, onMessage) {
        ...
        while() { }, true:  });
    {
        ...
        await;
        sleep(5000 + Math.round(Math.random() * 8000));
    }
}
;
async;
        ...
        if (isProxy()) {
            ...
            asyncForEach(allOwners, async(c), {
                ...
                if(c, publicKey =  && c.publicKey.length === 66) {
                    await;
                    lazyClient(c.publicKey, host, onMessage);
                    await;
                    subExtraHostsForTenant(c.id, c.publicKey, onMessage); // 1 is the tenant id on non-proxy
                }
            });
        }
        else {
            ...
            await;
            lazyClient(info.identity_pubkey, host, onMessage);
...
function updateTribeStats(myPubkey) {
    ...
    await;
    asyncForEach(myTribes, async(tribe), {
        try: {
...
function edit({ uuid, host, name, description, tags, img, price_per_message, price_to_join, owner_alias, escrow_amount, escrow_millis, unlisted, is_private, app_url, feed_url, deleted, owner_route_hint, owner_pubkey, }) {
    try {
        ...
        if (config.tribes_insecure)
            protocol = "http";
        await;
        fetch(protocol + "://" + host + "/tribe?token=" + token, {
            method: "PUT",
            body: JSON.stringify({
                uuid,
                name,
...
function delete_tribe(uuid, owner_pubkey) {
    ...
    try {
        ...
        if (config.tribes_insecure)
            protocol = "http";
        await;
        fetch(`${protocol}://${host}/tribe/${uuid}?token=${token}`, {
            method: "DELETE",
        });
    }
    catch (e) {
...
function putActivity(uuid, host, owner_pubkey) {
    try {
        ...
        if (config.tribes_insecure)
            protocol = "http";
        await;
        fetch(`${protocol}://${host}/tribeactivity/${uuid}?token=` + token, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
        });
    }
...
function putstats({ uuid, host, member_count, chatId, owner_pubkey, }) {
    ...
    try {
        ...
        if (config.tribes_insecure)
            protocol = "http";
        await;
        fetch(protocol + "://" + host + "/tribestats?token=" + token, {
            method: "PUT",
            body: JSON.stringify({
                uuid,
                member_count,
