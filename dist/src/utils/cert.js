...
function validateCert(port, data, endpoint, apiKey) {
    ...
    await;
    requestValidation(data.id, apiKey);
...
function getCertificate(domain, port, save_ssl) {
    ...
    await;
    validateCert(port, res, endpoint, apiKey);
    ...
    if (save_ssl === true) {
        ...
        await;
        writeFile(__dirname + "/zerossl/tls.cert", certData["certificate.crt"], function (err) {
            if (err) {
        ...
        await;
        writeFile(__dirname + "/zerossl/ca.cert", certData["ca_bundle.crt"], function (err) {
            if (err) {
                return console.log(err);
            }
        ...
        await;
        writeFile(__dirname + "/zerossl/tls.key", forge.pki.privateKeyToPem(keys.privateKey), function (err) {
            if (err) {
                return console.log(err);
            }
